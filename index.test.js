...
describe("Checker", () => {
  ...
  it("Validates subsequents dates", () => {
    expect(checkDate("0001_auto_20200101_0001.py", startDate)).toBe(
      "0001_auto_20200101_0001.py",
    );
  });
...
describe("Action", () => {
  ...
  it("Throws error on invalid type", () => {
    const result = cp.spawnSync("node", [innerPath], {
      env: { ...process.env, INPUT_TYPE: "invalid" },
    });
    expect(result.status).toBe(1);
    expect(result.stdout.toString()).toBe(
      '::error::Invalid type provided, correct choices are "app" or "project".\n',
    );
  });
  ...
  it("Throws error on invalid project", () => {
    ...
    expect(stdout).not.toContain(
      "test_files/project/app/migrations/0001_auto_20191231_0000.py",
    );
    expect(stdout).toContain(
      "test_files/project/app/migrations/0002_auto_20200101_0100.py",
    );
  });
